...
cmd_start() {
  ...
  for ((i = 0; i < 50; i++)); do [[ -f "$vfkit_pid" ]] && break; sleep 0.1; done
...
cmd_stop() {
  if running; then
    ...
    local i
    for ((i = 0; i < 30; i++)); do running || break; sleep 1; done
    running && { echo "graceful stop timed out — killing vfkit."; kill -9 "$(cat "$vfkit_pid")" 2>/dev/null || true; }
  fi
...
cmd_status() {
  if running; then echo "vfkit:   running (pid $(cat "$vfkit_pid"))"
  else echo "vfkit:   stopped"; fi
  if pid_alive "$gvproxy_pid"; then echo "gvproxy: running (pid $(cat "$gvproxy_pid"))"
